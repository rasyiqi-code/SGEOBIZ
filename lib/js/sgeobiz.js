/**
 * This file holds SGEOBIZ SEO plugin's JS code.
...
/**
 * Holds SGEOBIZ SEO values in an object to avoid polluting global namespace.
 *
 * @since 2.2.4
...
window.sgeobiz = function () {
	...
	function ampHTMLtoText( str ) {
		deprecatedFunc( 'sgeobiz.ampHTMLtoText', '5.1.0' );
	...
	function setAjaxLoader( target ) {
		...
		target?.classList.add( 'sgeobiz-loading' );
	}
	...
	function unsetAjaxLoader( target, success ) {
		...
		let newClass = 'sgeobiz-success',
			fadeTime = 2500;

		if ( ! success ) {
			newClass = 'sgeobiz-error';
			fadeTime = 5000;
		}
		...
		if ( target ) {
			target.classList.remove( 'sgeobiz-loading', 'sgeobiz-error', 'sgeobiz-success', 'sgeobiz-unknown' );
			target.classList.add( newClass );
			sgeobizUI.fadeOut( target, fadeTime );
		}
	}
	...
	function deprecatedFunc( name, version, replacement ) {
		// This will be removed during minification, because we have "removeConsole" enabled via babel-sgeobiz.
		console.warn(
			`[DEPRECATED]: ${name} is deprecated${version ? ` since SGEOBIZ SEO ${version}` : ''}.${replacement ? ` Use ${replacement} instead.` : ''}`
		);
	}
	...
	function dispatchAtInteractive( element, eventName ) {
		...
		if ( ! _loadedDispatchEvent ) {
			document.body.addEventListener( 'sgeobiz-interactive', _loopDispatchAtInteractive );
	 ...
	 * @since 4.1.2
	 * @access public
	 * @todo deprecate 5.2: Move it to sgeobizUI instead. Also split the trigger and the dispatch into two functions,
	 *       so that we need not create a constant function.
	 *
	 * @function
	 */
	const triggerNoticeReset = sgeobizUtils.debounce(
		() => { document.body.dispatchEvent( new CustomEvent( 'sgeobiz-reset-notice-listeners' ) ) },
		100, // Magic number. Low enough not to cause ignored clicks, high enough not to cause lag.
	);

	let _throttleResize = false;
	const _debounceResize        = sgeobizUtils.debounce( () => { _throttleResize = false }, 50 ),
		  _debounceResizeTrigger = sgeobizUtils.debounce( _triggerResize, 50 );
	/**
	 * Dispatches sgeobiz-resize event on window during resize events.
	 *
	...
	function _triggerResize() {
		...
		if ( _throttleResize ) {
			...
			dispatchEvent( new CustomEvent( 'sgeobiz-resize' ) );
		}
	}

	let isInteractive = false;
	/**
	 * Dispatches sgeobiz-interactive event once.
	 *
	 * This fires as soon as all SGEOBIZ script are done loading. A few more may load here that rely on user interaction.
	 ...
	 *
	 * Example: jQuery( document.body ).on( 'sgeobiz-interactive', myFunc );
	 * Or:      document.body.addEventListener( 'sgeobiz-interactive', myFunc );
	 *
	 * @since 4.1.1
	 * @access private
	 */
	function _triggerInteractive() {
		if ( ! isInteractive ) {
			isInteractive = true;
			document.body.dispatchEvent( new CustomEvent( 'sgeobiz-interactive' ) );
		}
	}

	/**
	 * Dispatches sgeobiz-ready event.
	 *
	 * This fires as soon as all SGEOBIZ scripts have registered their interactions.
	 ...
	 *
	 * Example: jQuery( document.body ).on( 'sgeobiz-ready', myFunc );
	 * Or:      document.body.addEventListener( 'sgeobiz-ready', myFunc );
	 *
	 * @since 2.9.0
	 * @access private
	 */
	function _triggerReady() {
		document.body.dispatchEvent( new CustomEvent( 'sgeobiz-ready' ) );
	}

	/**
	 * Dispatches 'sgeobiz-onload' event.
	 *
	 * This fires as soon as all SGEOBIZ scripts are loaded.
	 * Use case: User still sees a white screen, window has yet to be painted.
	 *
	 * You should not work asynchronously here.
	 *
	 * Example: jQuery( document.body ).on( 'sgeobiz-onload, myFunc );
	 * Or:      document.body.addEventListener( 'sgeobiz-onload', myFunc );
	 *
	 * @since 3.1.0
	 * @access private
	 */
	function _triggerOnLoad() {
		document.body.dispatchEvent( new CustomEvent( 'sgeobiz-onload' ) );
	}
	...
	function _doReady() {
		...
		// Triggers sgeobiz-onload event.
		_triggerOnLoad();

		// Trigger sgeobiz-ready event.
		_triggerReady();

		_isReady = true;

		// Trigger sgeobiz-interactive event. 'load' might be too late 'cause images are loading (slow 3G)...
	...
	return Object.assign( {
		...
		load: () => {
			 ...
			 * such as any page on sgeobiz.com; which benefit from considering load order & inline scripts, making for seamless rendering.
			 *
