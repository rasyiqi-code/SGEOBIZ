...
window.sgeobizSettings = function( $ ) {
	...
	function _initSchemaSettings() {
		...
		const presenceToggle   = document.getElementById( _getSettingsId( presenceTab.id ) ),
			  presenceTabRadio = document.getElementById( `sgeobiz-social-tab-${presenceTab.tab}` );
...
jQuery(document).ready(function($) {
    ...
    // Build modern layout wrapper
    var dashboardHtml = 
        '<div class="sgeobiz-dashboard">' +
        '    <div class="sgeobiz-sidebar">' +
        '        <div class="sgeobiz-sidebar-brand">' +
        '             <span class="dashicons dashicons-admin-generic"></span> SGEOBIZ SEO' +
        '        </div>' +
        '        <ul class="sgeobiz-sidebar-menu"></ul>' +
        '        <div class="sgeobiz-sidebar-footer">v' + (window.sgeobizSettings.l10n?.ver || '1.0.0') + '</div>' +
        '    </div>' +
    ...
    $postboxes.each(function(index) {
        ...
        var activeClass = isTabActive ? 'active' : '';
        $menu.append('<li class="sgeobiz-sidebar-item ' + activeClass + '" data-target="#' + id + '">' + title + '</li>');

        // Move to container
        $postbox.appendTo($container);

        if (isTabActive) {
            $postbox.addClass('sgeobiz-active-postbox').show();
        } else {
            $postbox.hide();
        }
    });
    ...
    // Tab click handler
    $('.sgeobiz-sidebar-item').on('click', function() {
        var $item = $(this);
        var target = $item.data('target');

        $('.sgeobiz-sidebar-item').removeClass('active');
        $item.addClass('active');

        $postboxes.hide().removeClass('sgeobiz-active-postbox');
        $(target).fadeIn(200).addClass('sgeobiz-active-postbox');

        // Update browser address bar query parameter dynamically without page reload
        var newSlug = Object.keys(sectionMap).find(function(key) { return sectionMap[key] === target; });
        if (newSlug) {
            var newUrl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?page=sgeobiz-seo-settings&section=' + newSlug;
