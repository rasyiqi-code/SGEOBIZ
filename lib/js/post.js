/**
 * This file holds SGEOBIZ SEO plugin's JS code for the Post SEO Settings.
...
/**
 * Holds sgeobizPost values in an object to avoid polluting global namespace.
 *
 * @since 4.0.0
 *
 * @constructor
 */
window.sgeobizPost = function () {
	...
	function _doFlexResizeListener() {
		...
		if ( ! document.querySelector( '.sgeobiz-flex' ) ) return;

		const wrapper = document.getElementById( 'sgeobiz-flex-inpost-tabs-wrapper' );

		const overflowAnimationFrame = new Map();

		const calculateTextOverflow = target => {

			const innerWrap = target.querySelector( '.sgeobiz-flex-nav-tab-inner' ),
				  navNames  = target.querySelectorAll( '.sgeobiz-flex-nav-name' );
		...
		window.addEventListener( 'sgeobiz-flex-resize', prepareCalculateTextOverflow );
	...
	function _initTabs() {
		sgeobizTabs.initStack(
			'sgeobizSettings',
			{
				tabToggledEvent: new CustomEvent( 'sgeobiz-flex-tab-toggled' ),
				HTMLClasses:     {
					wrapper:          'sgeobiz-flex-nav-tab-wrapper',
					tabRadio:         'sgeobiz-flex-nav-tab-radio',
					tabLabel:         'sgeobiz-flex-nav-tab-label',
					activeTab:        'sgeobiz-flex-tab-active', // change to sgeobiz-flex-nav-tab-active?
					// TODO make this sgeobiz-flex-tab-active-content (force -content affix?)
					activeTabContent: 'sgeobiz-flex-tab-content-active',
				},
	...
	function _registerPostPrivacyListener( callback ) {
		// Block Editor.
		document.addEventListener( 'sgeobiz-updated-block-editor-visibility', event => callback( event.detail.value ) );

		// Debounce the callback for Classic Editor, because toggling visibility will also trigger a password input event.
		callback = sgeobizUtils.debounce( callback, 20 ); // Magic number. The duplicate event happens in under a few ms; this is also imperceptible.
	...
	function _initVisibilityListeners() {
		...
		if ( sgeobizCanonical.usingPermalinks && canonicalInput ) {
			...
			const writePostname = sgeobizCanonical.structIncludes( _canonicalId, '%postname%' );
			const writeDate     = sgeobizCanonical.structIncludes( _canonicalId, [ '%year%', '%monthnum%', '%day%', '%hour%', '%minute%', '%second%' ] );
			const writeTerm     = {};
			const writeAuthor   = sgeobizCanonical.structIncludes( _canonicalId, '%author%' );
			...
			// Unpack post slugs.
			if ( writePostname ) {
				sgeobizPostSlugs.store( state.parentPostSlugs );
			...
			state.supportedTaxonomies.forEach( taxonomy => {
				writeTerm[ taxonomy ] = sgeobizCanonical.structIncludes( _canonicalId, `%${taxonomy}%` ); // Should always be true...
			} );
			// Unpack term slugs per taxonomy.
			for ( const [ taxonomy, terms ] of Object.entries( state.parentTermSlugs ) ) {
				sgeobizTermSlugs.store( terms, taxonomy );
				termSlugs[ taxonomy ] = terms.map( term => term.slug );
			}

			// Unpack author slugs.
			if ( writeAuthor ) {
				sgeobizAuthorSlugs.store( state.authorSlugs );
			...
			const updateCanonical = () => {
				if ( writePostname ) {
					...
					if ( postSlug.length ) {
						...
						activeSlug = sgeobizCanonical.sanitizeSlug( postSlug.substring( 0, 200 ) );
					...
					if ( ! activeSlug.length && postTitle.length )
						activeSlug = sgeobizCanonical.sanitizeSlug( postTitle.substring( 0, 200 ) );
			...
			document.addEventListener(
				...
				async event => {
					...
					switch ( event.detail.type ) {
						...
						case 'parent':
							if ( writePostname ) {
								parentSlugs = await sgeobizPostSlugs.get( event.detail.postData.get( 'parent' ) );
								queueUpdateCanonical();
							}
							break;
						case 'author':
							if ( writeAuthor ) {
								authorSlug = await sgeobizAuthorSlugs.get( event.detail.postData.get( 'author' ) );
								queueUpdateCanonical();
							}
							break;
						case 'date':
			...
			if ( Object.values( writeTerm ).includes( true ) ) {
				const updateParentTermSlugsViaPrimary = sgeobizUtils.debounce(
					async event => {

						const taxonomy = event.detail.taxonomy;

						if ( writeTerm[ taxonomy ] ) {
							termSlugs[ taxonomy ] = await sgeobizTermSlugs.get( event.detail.id, taxonomy );
							queueUpdateCanonical();
						}
					},
					100, // Magic number. High enough to prevent self-DoS, low enough to be responsive.
				);
				document.addEventListener( `sgeobiz-updated-primary-term`, updateParentTermSlugsViaPrimary );
			}
			...
			if ( ! l10n.params.isBlockEditor ) {
				if ( writePostname ) {
					...
					if ( parentIdInput ) {
						const updateParentSlug = sgeobizUtils.debounce(
							async () => {
								parentSlugs = await sgeobizPostSlugs.get( parentIdInput.value );
								queueUpdateCanonical();
							},
							100, // Magic number. High enough to prevent self-DoS, low enough to be responsive.
						);
				...
				if ( writeAuthor ) {
					...
					if ( authorIdInput ) {
						...
						const updateAuthor = sgeobizUtils.debounce(
							async () => {
								authorSlug = await sgeobizAuthorSlugs.get( authorIdInput.value );
								queueUpdateCanonical();
							},
							100, // Magic number. High enough to prevent self-DoS, low enough to be responsive.
						);
			...
			const setRobotsDefaultIndexingState = visibility => {
				...
				if ( _defaultIndexOption )
					_defaultIndexOption.innerHTML = noindexSelect.dataset.defaultI18n.replace(
						'%s',
						sgeobiz.escapeString( sgeobiz.decodeEntities( indexDefaultValue ) ),
					);
	...
	function _initTitleListeners() {
		...
		const state = JSON.parse( document.getElementById( `sgeobiz-title-data_${_titleId}` )?.dataset.state || 0 );
		...
		const updateDefaultTitle = val => {
			...
			sgeobizTitle.updateStateOf(
				_titleId,
				'defaultTitle',
				( sgeobizTitle.stripTitleTags ? sgeobiz.stripTags( val ) : val ) || sgeobizTitle.untitledTitle,
			);
		}
		...
		if ( ! l10n.params.isFront ) {
			...
			document.addEventListener(
				'sgeobiz-updated-block-editor-title',
	...
	function _initDescriptionListeners() {
		...
		const state = JSON.parse( document.getElementById( `sgeobiz-description-data_${_descId}` )?.dataset.state || 0 );

		if ( state ) {
			sgeobizDescription.updateStateOf( _descId, 'allowReferenceChange', ! state.refDescriptionLocked );
		...
		const setDescriptionVisibility = visibility => {
			let oldUseDefaultDescription = sgeobizDescription.getStateOf( _descId, 'useDefaultDescription' ),
			...
			if ( useDefaultDescription !== oldUseDefaultDescription )
				sgeobizDescription.updateStateOf( _descId, 'useDefaultDescription', useDefaultDescription );
		}
	 ...
	 *
	 * @since 4.0.0
	 * @since 4.1.0 Removed postbox-toggled listener, since sgeobiz-flex-resize is all-encapsulating now.
	 * @access private
	 */
	function _initGeneralListeners() {
		...
		document.getElementById( 'sgeobiz-flex-inpost-tab-general' )
			?.addEventListener( 'sgeobiz-flex-tab-toggled', enqueueGeneralInputListeners );
		window.addEventListener( 'sgeobiz-flex-resize', enqueueGeneralInputListeners );
	}
	...
	function _initUpdateMetaBox() {
		...
		const seobar = document.querySelector( '.sgeobiz-seo-bar' );

		// We only use this because it looks nice. The rest is implied via the counter updates.
		const seobarAjaxLoader = document.querySelector( '#sgeobiz-doing-it-right-wrap .sgeobiz-ajax' );

		const imageUrl = document.getElementById( 'autodescription_socialimage-url' );

		const _ogDescription = sgeobizSocial.getInputInstance( _socialGroup )?.inputs?.ogDesc;
		const _twDescription = sgeobizSocial.getInputInstance( _socialGroup )?.inputs?.twDesc;
		...
		const onSuccess = response => {
			...
			response = sgeobiz.convertJSONResponse( response );
			...
			setTimeout(
				() => {
					sgeobizDescription.updateStateOf( _descId, 'defaultDescription', response.data.metadescription.trim() );

					const socialDefaults = sgeobizSocial.getStateOf( _socialGroup, 'defaults' );
					socialDefaults.ogDesc = response.data.ogdescription.trim();
					socialDefaults.twDesc = response.data.twdescription.trim();
					sgeobizSocial.updateStateOf( _socialGroup, 'defaults', socialDefaults );

					if ( imageUrl ) {
						// Is this necessary? It's safer than assuming, though :)
						imageUrl.placeholder = sgeobiz.decodeEntities( response.data.imageurl );
			...
			seobar && sgeobizUI.fadeOut(
				seobar,
				fadeTime,
				() => {
					seobarAjaxLoader && sgeobiz.unsetAjaxLoader( seobarAjaxLoader, true );
		...
		const onFailure = () => {
			seobarAjaxLoader && sgeobiz.unsetAjaxLoader( seobarAjaxLoader, false );
		}

		document.addEventListener(
			'sgeobiz-gutenberg-onsave',
			() => {
				// Reset ajax loader, we only do that for the SEO Bar.
				seobarAjaxLoader && sgeobiz.resetAjaxLoader( seobarAjaxLoader );

				// Set ajax loader.
				seobarAjaxLoader && sgeobiz.setAjaxLoader( seobarAjaxLoader );

				wp.ajax.send(
					'sgeobiz_update_post_data',
					{
						data: {
	...
	/**
	 * Initializes settings scripts on SGEOBIZ-ready.
	 *
	 * @since 4.0.0
	...
	return Object.assign( {
		...
		load: () => {
			...
			document.body.addEventListener( 'sgeobiz-ready', _readySettings );
		},
	}, {
		l10n,
	} );
}();
window.sgeobizPost.load();
