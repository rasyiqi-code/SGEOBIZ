/**
 * This file holds SGEOBIZ SEO plugin's JS code for SGEOBIZ title fields.
...
/**
 * Holds sgeobizTitle values in an object to avoid polluting global namespace.
 *
...
window.sgeobizTitle = function () {
	 ...
	 * @type {String}
	 */
	const untitledTitle = sgeobiz.escapeString( l10n.params.untitledTitle );
	/**
	 * @since 4.1.0
	 * @access public
	 * @type {String}
	 */
	const protectedPrefix = sgeobiz.escapeString( l10n.i18n.protectedTitle );
	/**
	 * @since 4.1.0
	 * @access public
	 * @type {String}
	 */
	const privatePrefix = sgeobiz.escapeString( l10n.i18n.privateTitle );
	/**
	 * @since 4.1.0
	 * @access public
	 * @type {Boolean}
	...
	function _getHoverPrefixElement( id ) {
		return document.getElementById( `sgeobiz-title-placeholder-prefix_${id}` ) || document.createElement( 'span' );
	}
	/**
	 * @since 4.1.0
	 * @access private
	...
	function _getHoverAdditionsElement( id ) {
		return document.getElementById( `sgeobiz-title-placeholder-additions_${id}` ) || document.createElement( 'span' );
	}
	...
	function _getTitleReferences( id ) {
		return [ document.getElementById( `sgeobiz-title-reference_${id}` ) ];
	}
	...
	function _getTitleNaReferences( id ) {
		return [ document.getElementById( `sgeobiz-title-noadditions-reference_${id}` ) ];
	}
	...
	function _setReferenceTitle( event ) {
		...
		let text = sgeobiz.coalesceStrlen( allowReferenceChange && event.target.value.trim() )
				?? sgeobiz.coalesceStrlen( getStateOf( event.target.id, 'defaultTitle' ) )
				?? '';
		let textNa = text;
		...
		const referenceValue = sgeobiz.escapeString(
			sgeobiz.decodeEntities(
				sgeobiz.sDoubleSpace(
					sgeobiz.sTabs(
						sgeobiz.sSingleLine(
							text,
						)
		...
		const referenceNaValue = sgeobiz.escapeString(
			sgeobiz.decodeEntities(
				sgeobiz.sDoubleSpace(
					sgeobiz.sTabs(
						sgeobiz.sSingleLine(
	...
	function _updateAdditionsValue( id ) {
		...
		if ( getStateOf( id, 'addAdditions' ) ) {
			additionsValue = sgeobiz.escapeString( sgeobiz.decodeEntities( getStateOf( id, 'additionValue' ) ) );
	...
	function _updateHoverPlacement( event ) {
		...
		const offsetElement = document.getElementById( `sgeobiz-title-offset_${event.target.id}` );
		...
		if ( hasPrefixValue ) {
			...
			prefixWidth = hoverPrefixElement.getBoundingClientRect().width - ( hoverPrefixElement.dataset.sgeobizCorPad || 0 );
		...
		if ( hasAdditionsValue ) {
			...
			additionsWidth = hoverAdditionsElement.getBoundingClientRect().width - ( hoverAdditionsElement.dataset.sgeobizCorPad || 0 );
		...
		if ( hasPrefixValue ) {
			...
			hoverPrefixElement.dataset.sgeobizCorPad = prefixCorPad;
		}
		...
		if ( hasAdditionsValue ) {
			...
			hoverAdditionsElement.dataset.sgeobizCorPad = additionsCorPad;
		}
	...
	function _updateCounter( event ) {
		...
		sgeobizC?.updateCharacterCounter( {
			e:     counter,
	...
	function triggerCounter( id ) {
		if ( id ) {
			getInputElement( id )?.dispatchEvent( new CustomEvent( 'sgeobiz-update-title-counter' ) );
		} else {
			// We don't want it to loop infinitely. Check element.id value first.
	...
	/**
	 * Triggers meta title update, without affecting sgeobizAys change listeners.
	 *
	 * @since 4.0.0
	 * @since 4.1.0 Now allows for a first parameter to be set.
	 * @access public
	...
	function triggerUnregisteredInput( id ) {
		if ( 'sgeobizAys' in window ) {
			let wereSettingsChanged = sgeobizAys.areSettingsChanged();

			triggerInput( id );

			// Only reset if we polluted the change listener, and only if a change wasn't already registered.
			if ( ! wereSettingsChanged && sgeobizAys.areSettingsChanged() )
				sgeobizAys.reset();
		} else {
			triggerInput( id );
		}
	...
	function _focusTitleInput( event ) {
		...
		const type       = event.target.classList.contains( 'sgeobiz-title-placeholder-additions' ) ? 'additions' : 'prefix',
	...
	function _initAllTitleActions() {
		...
		window.addEventListener( 'sgeobiz-resize', _doResize );

		// When counters are updated, trigger an input; which will reassess them.
		window.addEventListener( 'sgeobiz-counter-updated', () => enqueueUnregisteredInputTrigger() );
	}
	...
	function _loadTitleActions( titleInput ) {
		...
		titleInput.addEventListener( 'sgeobiz-update-title-counter', _onUpdateCounterTrigger );
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-onload', _initAllTitleActions );
		},
	}, {
		setInputElement,
		getInputElement,
		...
		enqueueTriggerInput,
		triggerUnregisteredInput,
		enqueueUnregisteredInputTrigger, // FIXME: this should've been enqueueTriggerUnregisteredInput... deprecate in SGEOBIZ 5.2
	}, {
		l10n,
		untitledTitle,
		privatePrefix,
