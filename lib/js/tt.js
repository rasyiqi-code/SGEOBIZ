...
/**
 * Holds sgeobizTT (sgeobiz tooltip) values in an object to avoid polluting global namespace.
 *
 * @since 3.1.0
 *
 * @constructor
 */
window.sgeobizTT = function () {

	const _ttBase = 'sgeobiz-tooltip';
	const ttNames = {
		base:      _ttBase,
		item:      `${_ttBase}-item`,
		wrap:      `${_ttBase}-wrap`,
	...
	function _events( target ) {
		...
		return {
			set: () => {
				...
				target.addEventListener( 'sgeobiz-tooltip-update', _activeToolTipHandles.updateDesc );
	...
	function _initToolTips() {
		...
		// TODO move this test to the main sgeobiz object? This whole file doesn't rely on `window.sgeobiz` though.
		...
		try {
			( () => {
				...
				window.addEventListener( 'sgeobiz-tt-test-passive', null, options );
				window.removeEventListener( 'sgeobiz-tt-test-passive', null, options );
			} )();
		} catch ( e ) {
		...
		window.addEventListener( 'sgeobiz-tooltip-reset', init );
		triggerReset();
	}
	...
	function _renderTooltip( event, element, desc ) {
		...
		if ( boundaryTop > ( tooltip.getBoundingClientRect().top - tooltipHeight ) ) {
			tooltip.classList.add( 'sgeobiz-tooltip-down' );
	...
	function triggerReset() {
		...
		_debounceTriggerReset = setTimeout(
			() => window.dispatchEvent( new CustomEvent( 'sgeobiz-tooltip-reset' ) ),
	...
	function triggerUpdate( element ) {
		...
		const updateEvent = new CustomEvent( 'sgeobiz-tooltip-update' );
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-ready', _initToolTips );
		},
	}, {
		/**
		 * Copies internal public functions to sgeobizTT for public access.
