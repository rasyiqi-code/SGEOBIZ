/**
 * This file holds SGEOBIZ SEO plugin's JS code for Primary Term Selection in List Edit.
...
/**
 * SGEOBIZ SEO plugin
 * Copyright (C) 2025 SGEOBIZ (https://rasyiqi-code.github.io/SGEOBIZ/)
 *
...
/**
 * Holds sgeobizPTLE (sgeobiz primary term list edit) values in an object to avoid polluting global namespace.
 *
 * @since 5.1.3
 *
 * @constructor
 */
window.sgeobizPTLE = function () {
	...
	function _prepareQuickEditTerms( postId, primaryTerms ) {
		...
		const initQuickEditSelector = taxonomySlug => {
			...
			const selectorWrapId   = `sgeobiz-pt-le-${taxonomySlug}-${postId}`,
			...
			const showSelectorWrap = () => {
				...
				if ( ! selectElement ) {
					const markup = _renderTemplate(
						'sgeobiz-primary-term-selector-quick',
						{
	...
	function _prepareBulkEditTerms() {
		...
		const initBulkSelector = taxonomySlug => {
			...
			const selectorWrapId = `sgeobiz-pt-le-${taxonomySlug}-bulk`;
			...
			const showSelectorWrap = () => {
				...
				if ( ! selectElement ) {
					const markup = _renderTemplate(
						'sgeobiz-primary-term-selector-bulk',
						{
							wrapId: selectorWrapId,
							selectId,
							selectName: selectId,
			...
			const repopulateSelect = () => {
				...
				sgeobiz.selectByValue( selectElement, nextValue );
			}
			termCheckboxes.subscribe( () => {
				if ( termCheckboxes.getInputsChecked().length > 1 ) {
					if ( ! selectorWrapShown ) {
