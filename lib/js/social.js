/**
 * This file holds SGEOBIZ SEO plugin's JS code for the Social Input Settings.
...
/**
 * Holds sgeobizSocial values in an object to avoid polluting global namespace.
 *
 * @since 4.0.0
...
window.sgeobizSocial = function () {
	...
	function _tickState( group, part ) {
		switch ( part ) {
			case 'addAdditions':
				...
				titleRef && sgeobizTitle.enqueueUnregisteredInputTrigger( titleRef );
				break;
			default:
				break;
		}
	...
	function setInputInstance( group, titleRef, descRef ) {
		...
		const _getElement = type => document.querySelector(
			`[data-sgeobiz-social-group="${group}"][data-sgeobiz-social-type="${type}"]`,
		);
		...
		const refs = {
			...
			title:      document.getElementById( `sgeobiz-title-reference_${titleRef}` ),
			titleNa:    document.getElementById( `sgeobiz-title-noadditions-reference_${titleRef}` ),
			desc:       document.getElementById( `sgeobiz-description-reference_${descRef}` ),
		}
	...
	function _loadTitleActions( group ) {
		...
		const getActiveValue = what => {
			...
			while ( 'undefined' !== typeof val && ! val.length ) {
				...
				if ( val?.length )
					val = sgeobiz.sDoubleSpace( sgeobiz.sTabs( sgeobiz.sSingleLine( val ) ) );
			}
		...
		const setPlaceholders = () => {
			...
			if ( inputs.ogTitle )
				inputs.ogTitle.placeholder = locks.ogTitle || phLocks.ogTitle
					? sgeobiz.decodeEntities( getState( 'defaults' ).ogTitle )
					: sgeobiz.decodeEntities( getActiveValue( 'meta' ) );
			if ( inputs.twTitle )
				inputs.twTitle.placeholder = locks.twTitle || phLocks.twTitle
						? sgeobiz.decodeEntities( getState( 'defaults' ).twTitle )
						: sgeobiz.decodeEntities( getActiveValue( 'og' ) );
		}
		const updateCounter = ( target, text, type ) => {
			const counter = document.getElementById( `${target.id}_chars` );
			counter && sgeobizC?.updateCharacterCounter( {
				e:     counter,
				text:  text,
				field: 'title',
				type:  type,
		...
		const updateRefTitle = sgeobizUtils.debounce(
			() => {
				setPlaceholders();
		...
		const updateTitle = sgeobizUtils.debounce(
			() => {
				setPlaceholders();
				updateSocialCounters();
			},
	...
	function _loadDescriptionActions( group ) {
		...
		const setPlaceholders = () => {
			...
			if ( inputs.ogDesc )
				inputs.ogDesc.placeholder = locks.ogDesc || phLocks.ogDesc
					? sgeobiz.decodeEntities( getState( 'defaults' ).ogDesc )
					: sgeobiz.decodeEntities( getActiveValue( 'meta', 'og' ) );
			// Security OK. All getActiveValue is escaped.
			if ( inputs.twDesc )
				inputs.twDesc.placeholder = locks.twDesc || phLocks.twDesc
					? sgeobiz.decodeEntities( getState( 'defaults' ).twDesc )
					: sgeobiz.decodeEntities( getActiveValue( 'og', 'twitter' ) );
		}
		const updateCounter = ( target, text, type ) => {
			const counter = document.getElementById( `${target.id}_chars` );
			counter && sgeobizC?.updateCharacterCounter( {
		...
		const updateRefDesc = sgeobizUtils.debounce(
			() => {
				setPlaceholders();
				updateSocialCounters();
			},
		...
		const updateDesc = sgeobizUtils.debounce(
			() => {
				setPlaceholders();
				updateSocialCounters();
			},
