/**
 * This file holds SGEOBIZ SEO plugin's JS code for Image Selection and Cropping.
...
/**
 * Holds sgeobizMedia values in an object to avoid polluting global namespace.
 *
 * @since 3.1.0
 *
 * @constructor
 */
window.sgeobizMedia = function () {
	...
	function _openImageEditor( event ) {
		...
		const onDone = () => {
			...
			'sgeobizAys' in window && sgeobizAys.registerChange();
		};
	...
	function _appendRemoveButton( target, animate ) {
		...
		button.title             = sgeobiz.decodeEntities( l10n.labels[ inputType ].imgRemoveTitle );
		button.innerHTML         = sgeobiz.escapeString( sgeobiz.decodeEntities( l10n.labels[ inputType ].imgRemove ) );

		button.classList.add( 'sgeobiz-remove-image-button', ...( JSON.parse( target.dataset?.buttonClass || 0 )?.remove || [] ) );

		target.insertAdjacentElement( 'afterend', button );

		if ( animate )
			sgeobizUI.fadeIn( button, 250 ); // Magic number: thickbox doesn't quickly disappear; so double the default.
	...
	function _extendCropper() {
		 ...
		 */
		const SGEOBIZView = View.Cropper.extend( {
			className:   'crop-content sgeobiz-image',
			ready:       function () {
		 ...
		 */
		const SGEOBIZCropper = wp.media.controller.Cropper.extend( {
			createCropContent: function () {
				this.cropperView = new SGEOBIZView( {
					controller: this,
		...
		SGEOBIZCropper.prototype.control = {};

		Cropper = SGEOBIZCropper;
	}
	...
	function _checkImageEditorInput() {
		...
		document.querySelectorAll( '.sgeobiz-set-image-button' ).forEach(
			element => {
	...
	function _resetImageEditorSetActions() {
		document.querySelectorAll( '.sgeobiz-set-image-button' ).forEach( el => {
	...
	function _resetImageEditorRemovalActions() {
		document.querySelectorAll( '.sgeobiz-remove-image-button' ).forEach( el => {
	 ...
	 *
	 * @since 3.1.0
	 * @see sgeobizemMedia.setupImageEditorActions() (Extension Manager plugin)
	 * @access private
	 */
	function _setupImageEditorActions() {
		...
		document.querySelectorAll( '.sgeobiz-enable-media-if-js' ).forEach( el => {
			el.disabled = false;
			el.classList.remove( 'sgeobiz-enable-media-if-js' );
		} );
	...
	function _updateImageNotifications( event ) {
		...
		const firstLoad = ! ( event.target.dataset.sgeobizNotificationsLoaded || false );
		...
		const updateToolTip = () => {
			...
			event.target.dataset.sgeobizCurrentInputTime = currentUpdateTime; // note: this will be a string.
			...
			const writeTooltips = success => {
				...
				if ( currentUpdateTime !== +event.target.dataset.sgeobizCurrentInputTime ) return;
				...
				if ( imageWarning ) {
					...
					if ( ! warning.length ) {
						...
						if ( ext?.length ) {
							if (
								...
								warning     = l10n.warning.i18n.extForbidden.replace( '%s', sgeobiz.escapeString( ext ) );
								warningType = 'error';
							} else if (
								   l10n.warning.warnedTypes?.[ imageType ]?.hasOwnProperty( ext )
								|| l10n.warning.warnedTypes?.all?.hasOwnProperty( ext )
							) {
								warning = l10n.warning.i18n.extWarned.replace( '%s', sgeobiz.escapeString( ext ) );
							}
						}
					...
					if ( warning.length ) {
						...
						imageWarning.classList.toggle( 'sgeobiz-media-warning', warningType === 'warning' );
						imageWarning.classList.toggle( 'sgeobiz-media-error', warningType === 'error' );
		...
		event.target.dataset.sgeobizNotificationsLoaded = true;
	...
	function _prepareTooltip() {
		document.querySelectorAll( '.sgeobiz-image-notifications' ).forEach( el => {
	 ...
	 * @function
	 */
	const resetImageEditorActions = sgeobizUtils.debounce( _setupImageEditorActions, 500 );
	...
	return Object.assign( {
		...
		load: () => {
			...
			document.body.addEventListener( 'sgeobiz-ready', _setupImageEditorActions );
		},
	}, {
