/**
 * This file holds SGEOBIZ SEO plugin's JS code for WordPress List Edit adjustments.
...
/**
 * Holds sgeobizLe values in an object to avoid polluting global namespace.
 *
 * @since 4.0.0
 *
 * @constructor
 */
window.sgeobizLe = function () {

	/**
	 * The current default fields data obtained via sgeobizLeData.
	...
	/**
	 * The current default post data obtained via sgeobizLePostData.
	 ...
	 * @function
	 */
	const _dispatchUpdate = sgeobizUtils.debounce(
		() => { document.dispatchEvent( new CustomEvent( 'sgeobizLeUpdated' ) ); },
	...
	function _setInlinePostValues() {
		for ( const option in fieldsData ) {
			...
			if ( params.isSelect ) {
				sgeobiz.selectByValue( element, params.value );

				// Do `sprintf( 'Default (%s)', params.default )`.
				const _default = element.querySelector( '[value="0"]' );
				if ( _default )
					_default.innerHTML = _default.innerHTML.replace( '%s', sgeobiz.escapeString( sgeobiz.decodeEntities( params.default ) ) );
			} else {
				element.value = sgeobiz.decodeEntities( params.value );

				if ( params.placeholder?.length )
					element.placeholder = sgeobiz.decodeEntities( params.placeholder );
			}
		}
	}
	...
	function _prepareTitleInput( id ) {
		...
		const data = JSON.parse( document.getElementById( `sgeobizLeTitleData[${id}]` )?.dataset.leTitle || 0 );

		if ( data ) {
			sgeobizTitle.updateStateOf( titleId, 'allowReferenceChange', ! data.refTitleLocked );
			sgeobizTitle.updateStateOf( titleId, 'defaultTitle', data.defaultTitle.trim() );
			sgeobizTitle.updateStateOf( titleId, 'addAdditions', data.addAdditions );
			sgeobizTitle.updateStateOf( titleId, 'additionValue', data.additionValue.trim() );
			sgeobizTitle.updateStateOf( titleId, 'additionPlacement', data.additionPlacement );
		}
		...
		if ( 'post' === _editType ) {
			...
			const setTitleVisibilityPrefix = () => {
				...
				sgeobizTitle.updateStateOf( titleId, 'prefixValue', prefixValue );
			}
		...
		const setDefaultTitle = event => {
			...
			let defaultTitle = (
					sgeobizTitle.stripTitleTags
						? sgeobiz.stripTags( inputTitle )
						: inputTitle
				) || sgeobizTitle.untitledTitle;
			...
			// TODO figure out if this is necessary. sgeobizTitle also escapes...
			defaultTitle = sgeobiz.escapeString( sgeobiz.decodeEntities( defaultTitle.trim() ) );

			sgeobizTitle.updateStateOf( titleId, 'defaultTitle', defaultTitle );
		}
	...
	function _prepareDescriptionInput( id ) {
		...
		const state = JSON.parse( document.getElementById( `sgeobizLeDescriptionData[${id}]` )?.dataset.leDescription || 0 );
		if ( state ) {
			sgeobizDescription.updateStateOf( descId, 'allowReferenceChange', ! state.refDescriptionLocked );
	...
	function _prepareVisibilityInput( id ) {
		...
		const state = JSON.parse( document.getElementById( `sgeobizLeCanonicalData[${id}]` )?.dataset.leCanonical || 0 );
		...
		if ( sgeobizCanonical.usingPermalinks && canonicalInput && inlineEditWrap ) switch ( _editType ) {
			case 'post': { // rescope for there's reused block-scoped variables with 'tax'.
				...
				const writePostname = sgeobizCanonical.structIncludes( canonicalId, '%postname%' );
				const writeDate     = sgeobizCanonical.structIncludes( canonicalId, [ '%year%', '%monthnum%', '%day%', '%hour%', '%minute%', '%second%' ] );
				const writeTerm     = {};
				const writeAuthor   = sgeobizCanonical.structIncludes( canonicalId, '%author%' );
				...
				state.supportedTaxonomies.forEach( taxonomy => {
					writeTerm[ taxonomy ] = sgeobizCanonical.structIncludes( canonicalId, `%${taxonomy}%` ); // Should always be true...
				} );
			...
			case 'tax': { // rescope for there's reused block-scoped variables with 'post'.
				...
				const writeTaxonomy = sgeobizCanonical.structIncludes( canonicalId, `%${taxonomy}%` );
		...
		if ( indexSelect ) {
			if ( 'post' === _editType ) {
				 ...
				 * @function
				 */
				const setRobotsDefaultIndexingState = sgeobizUtils.debounce(
					() => {
	...
	function _prepareTermInput( id ) {
		window.sgeobizPTLE._prepareQuickEditTerms( id, postData?.primaryTerms );
	}
	...
	function _prepareTermInputBulk() {
		window.sgeobizPTLE._prepareBulkEditTerms();
	}
	...
	function _setListeners() {
		document.addEventListener( 'sgeobizLeDispatchUpdate', _dispatchUpdate );
		document.addEventListener( 'sgeobizLeUpdated', _updated );
	}
	...
	function _hijackListeners() {
		...
		if ( _oldInlineEditPost ) {
			window.inlineEditPost.edit = function( id ) {
				...
				_editType  = 'post';
				fieldsData = JSON.parse( document.getElementById( `sgeobizLeData[${id}]` )?.dataset.le || 0 ) || {};
				postData   = JSON.parse( document.getElementById( `sgeobizLePostData[${id}]` )?.dataset.lePostData || 0 ) || {};
				...
				window.sgeobizC?.resetCounterListener();
	...
	return {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-onload', _setListeners );
			document.body.addEventListener( 'sgeobiz-onload', _hijackListeners );
		},
	};
}();
window.sgeobizLe.load();
