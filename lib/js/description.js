/**
 * This file holds SGEOBIZ SEO plugin's JS code for SGEOBIZ description fields.
...
/**
 * Holds sgeobizDescription values in an object to avoid polluting global namespace.
 *
 ...
 * @constructor
 */
window.sgeobizDescription = function () {
	...
	function _getDescriptionReferences( id ) {
		return [ document.getElementById( `sgeobiz-description-reference_${id}` ) ];
	}
	...
	function _setReferenceDescription( event ) {
		...
		let text = sgeobiz.coalesceStrlen( allowReferenceChange && event.target.value.trim() )
				?? sgeobiz.coalesceStrlen( useDefaultDescription && getStateOf( event.target.id, 'defaultDescription' ) )
				?? '';
	...
	function triggerCounter( id ) {
		if ( id ) {
			getInputElement( id )?.dispatchEvent( new CustomEvent( 'sgeobiz-update-description-counter' ) );
		} else {
			// We don't want it to loop infinitely. Check element.id value first.
	...
	/**
	 * Triggers description update, without affecting sgeobizAys change listeners.
	 *
	 * @since 4.0.0
	 * @since 4.1.0 Now allows for a first parameter to be set.
	 * @access public
	...
	function _loadDescriptionActions( descriptionInput ) {
		...
		descriptionInput.addEventListener( 'sgeobiz-update-description-counter', _onUpdateCounterTrigger );
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-onload', _initAllDescriptionActions );
		},
	}, {
		setInputElement,
		getInputElement,
...
window.sgeobizDescription.load();
