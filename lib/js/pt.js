/**
 * This file holds SGEOBIZ SEO plugin's JS code for Primary Term Selection.
...
/**
 * Holds sgeobizPT (sgeobiz primary term) values in an object to avoid polluting global namespace.
 *
...
window.sgeobizPT = function () {
	...
	function _primaryTermSelector( taxonomySlug ) {
		...
		const registerPostField = () => {
			// TODO create a new dataholder for SGEOBIZ instead?
			const wrap = document.getElementById( `${taxonomySlug}div` );

			if ( wrap ) {
				wrap.insertAdjacentHTML(
					'beforeend',
					wp.template( 'sgeobiz-primary-term-selector' )(
	...
	function _initPrimaryTerm() {
		...
		const initPrimaryTermSelector = taxonomySlug => {
			...
			const selectorWrapId = `sgeobiz-primary-term-${taxonomySlug}`,
				...
				selectElement.append(
					...termCheckboxes.getInputsChecked().map(
						el => {
							...
							option.label = sgeobiz.decodeEntities( el.parentElement.textContent.trim() );
							return option;
						},
					),
				);

				sgeobiz.selectByValue( selectElement, primaryTerm.get() );
			}
			...
			const showSelectorWrap = () => {
				...
				selectorWrap.classList.add( 'sgeobiz-primary-term-selector-wrap' );
				...
				sgeobizAys.registerChangeListener( selectElement, 'change' );
			}
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-onload', _initPrimaryTerm );
		},
	}, {
		l10n,
	} );
}();
window.sgeobizPT.load();
