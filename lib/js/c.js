/**
 * This file holds SGEOBIZ SEO plugin's JS code for pixel and character counters.
...
/**
 * Holds sgeobizC values in an object to avoid polluting global namespace.
 *
 * @since 4.0.0
 *
 * @constructor
 */
window.sgeobizC = function () {
	...
	const counterClasses = {
		0: 'sgeobiz-counter-zero',
		1: 'sgeobiz-counter-one',
		2: 'sgeobiz-counter-two',
		3: 'sgeobiz-counter-three',
	};
	...
	function updateCharacterCounter( test ) {
		...
		let el         = test.e,
			text       = sgeobiz.decodeEntities( test.text ),
			guidelines = l10n.guidelines[ test.field ][ test.type ].chars;

		let testLength = sgeobiz.getStringLength( text ),
			newClass   = '',
			exclaimer  = '';

		const classes = {
			bad:     'sgeobiz-count-bad',
			okay:    'sgeobiz-count-okay',
			good:    'sgeobiz-count-good',
			unknown: 'sgeobiz-count-unknown',
		};
	...
	function updatePixelCounter( test ) {
		...
		const bar    = wrap.querySelector( '.sgeobiz-pixel-counter-bar' ),
			  shadow = wrap.querySelector( '.sgeobiz-pixel-counter-shadow' );

		if ( ! bar || ! shadow )
			return;

		shadow.innerHTML = sgeobiz.escapeString( sgeobiz.decodeEntities( test.text ) );
		...
		const classes    = {
			bad:     'sgeobiz-pixel-counter-bad',
			okay:    'sgeobiz-pixel-counter-okay',
			good:    'sgeobiz-pixel-counter-good',
			unknown: 'sgeobiz-pixel-counter-unknown',
		};
		...
		bar.querySelector( '.sgeobiz-pixel-counter-fluid' ).style.width = newWidth;

		// Update tooltip and ARIA label.
		bar.dataset.desc = label;
		// Replace HTML tags with spaces. TODO see SGEOBIZ's PHP-code `strip_tags_cs()` for a better solution.
		...
		bar.setAttribute( 'aria-label', sgeobiz.escapeString( label.replace( /(<([^>]+)?>?)/ig, ' ' ) ) );

		sgeobizTT.triggerUpdate( bar );
	}
	...
	function triggerCounterUpdate() {
		window.dispatchEvent( new CustomEvent( 'sgeobiz-counter-updated' ) );
	}
	...
	function _counterUpdate() {
		...
		const target = '.sgeobiz-counter-wrap .sgeobiz-ajax';

		sgeobiz.resetAjaxLoader( target );
		sgeobiz.setAjaxLoader( target );

		wp.ajax.post(
			'sgeobiz_update_counter',
			{
				nonce: sgeobiz.l10n.nonces.edit_posts,
				val:   counterType,
			}
		).done( () => {
			sgeobiz.unsetAjaxLoader( target, true );
		} ).fail( () => {
			sgeobiz.unsetAjaxLoader( target, false );
		} );
	}
	...
	function resetCounterListener() {
		return document.querySelectorAll( '.sgeobiz-counter' ).forEach(
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-onload', _initCounters );
		},
	}, {
		updatePixelCounter,
