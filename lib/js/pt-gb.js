/**
 * This file holds SGEOBIZ SEO plugin's JS code for Primary Term Selection for Gutenberg.
 *
...
/**
 * Holds sgeobizPTGB (sgeobiz primary term Gutenberg) values in an object to avoid polluting global namespace.
 *
...
window.sgeobizPTGB = function () {
	...
	function _primaryTermSelector( taxonomySlug ) {
		...
		const registerPostField = () => {
			const wrap = document.getElementById( 'sgeobiz-gutenberg-data-holder' );
		...
		function primaryTermSelector( props ) {
			...
			const getSelectOptions = () => {
				return availableTerms.map(
					term =>
						...
						&& {
							value: term.id,
							label: sgeobiz.decodeEntities( term?.name ), // TODO, consider using https://github.com/WordPress/gutenberg/pull/47561/files.
						},
				).filter( Boolean ) || '';
			};
			...
			return createElement(
				SelectControl,
				{
					...
					value:     selection,
					className: 'sgeobiz-pt-gb-selector',
					onChange:  onChange,
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-ready', _initPrimaryTerm );
		},
	}, {
		l10n,
	} );
}();
window.sgeobizPTGB.load();
