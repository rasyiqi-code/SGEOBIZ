/**
 * This file holds UI code for SGEOBIZ's interface.
...
/**
 * SGEOBIZ SEO plugin
 * Copyright (C) 2024 - 2025 SGEOBIZ (https://rasyiqi-code.github.io/SGEOBIZ/)
 *
...
/**
 * Holds sgeobizUI (sgeobiz User Interface) values in an object to avoid polluting global namespace.
 *
 * @since 5.1.0
 *
 * @constructor
 */
window.sgeobizUI = function( $ ) {
	 ...
	 * @since 5.1.0 Moved to sgeobizUI.
	 * @access private
	 */
	function _initPostboxToggle() {

		// Get SGEOBIZ postboxes. Move this inside of the event for the "dynamic web"?
		let $postboxes = $( '.postbox[id^="autodescription-"], .postbox#sgeobiz-inpost-box' );
	...
	function _initNotices() {
		...
		/**
		 * Dismissible notices that use notice wrapper class .sgeobiz-notice.
		 *
		 * @since 2.6.0
		...
		const dismissNotice = event => {
			...
			const notice = event.target.closest( '.sgeobiz-notice' ),
			...
			if ( key && nonce ) {
				...
				wp.ajax.post(
					'sgeobiz_dismiss_notice',
					{
						sgeobiz_dismiss_key:   key,
						sgeobiz_dismiss_nonce: nonce,
					}
				);
			}
		}
		...
		const reset = () => {
			...
			document.querySelectorAll( '.sgeobiz-dismiss' ).forEach(
				el => { el.addEventListener( 'click', dismissNotice ) }
			);
		}
		/**
		 * @access private Use sgeobiz.triggerNoticeReset() instead.
		 */
		document.body.addEventListener( 'sgeobiz-reset-notice-listeners', reset );
		reset();
	}
	...
	async function fadeIn( element, duration = 125, cb = void 0, css = {} ) {
		...
		await sgeobizUtils.delay( duration );
	...
	function traceAnimation( element, name ) {
		...
		element.dataset.sgeobizIsAnimating = animation;

		return {
			unchanged:        () => animation === element.dataset.sgeobizIsAnimating,
			unsetIfUnchanged: () => {
				if ( animation === element.dataset.sgeobizIsAnimating )
	...
	return Object.assign( {
		 ...
		 */
		load: () => {
			document.body.addEventListener( 'sgeobiz-ready', _initPostboxToggle );
			document.body.addEventListener( 'sgeobiz-ready', _initNotices );
		},
	}, {
		/**
		 * Copies internal public functions to sgeobizUI for public access.
