/**
 * This file holds SGEOBIZ SEO plugin's JS code for the UI Tabs.
...
/**
 * SGEOBIZ SEO plugin
 * Copyright (C) 2020 - 2025 SGEOBIZ (https://rasyiqi-code.github.io/SGEOBIZ/)
 *
...
/**
 * Holds sgeobizTabs values in an object to avoid polluting global namespace.
 *
 * No jQuery. Cool.
 *
 * @since 4.1.3
 ...
 * @constructor
 */
window.sgeobizTabs = function () {

	/**
	 * @since 4.1.3
	 * @access protected (readonly)
	 * @see sgeobizTabs.getStack()
	...
	function toggleTo( stackId, target ) {
		...
		const setCorrectTab = async () => {
			...
			newContent.classList.add( stack.HTMLClasses.activeTabContent );
			unLockHeight();
			sgeobizUI.fadeIn( newContent, fadeInTimeout );

			// Resolve early at 2/3th of fade-in time, content should already be well visible.
			await sgeobizUtils.delay( fadeInTimeout * 2/3 );
		...
		const doPromise = () => new Promise( async resolve => {
			allContent.forEach( el => { sgeobizUI.fadeOut( el, fadeOutTimeout ) } );
			// Await fadeout before continuing (with fadeIn at setCorrectTab).
			await sgeobizUtils.delay( fadeOutTimeout );
	...
	function _toggle( stackId, event ) {
		...
		if ( previousToggle ) {
			...
			label && label.classList.remove( 'sgeobiz-no-focus-ring' );
		}
	 ...
	 * @since 4.1.3
	 * @access public
	 * @see sgeobizTabs.initStack() which registers the stack via param args.
	 *
	 * @param {String} stackId The stack ID for which to get the stack.
	 ...
	 * For an example, see `sgeobizSettings._initTabs()` and visit the SEO Settings page.
	 *
	 * @since 4.1.3
	 * @access public
	 * @see sgeobizTabs.initStack registers the stack.
	 *
	 * @param {String}             stackId The stack ID for which to get the stack.
	...
	function initStack( stackId, args ) {
		...
		const addNoFocusClass = event => event.currentTarget.classList.add( 'sgeobiz-no-focus-ring' );
